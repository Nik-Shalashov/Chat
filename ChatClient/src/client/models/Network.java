        ...
        Thread thread = new Thread(() -> {
           try {
               while (true) {
                    ...
                    if (msg.startsWith(CLIENT_MSG_PREFIX)) {
                        String[] parts = msg.split("\\s+", 3);
                        String sender = parts[1];
                        String messageBody = parts[2];

                        Platform.runLater(() -> chatController.appendMessage(String.format("%s: %s", sender, messageBody)));
                    } else if (msg.startsWith(SERVER_MSG_PREFIX)) {
                        String[] parts = msg.split("\\s+", 2);
                        Platform.runLater(() -> chatController.appendMessage(parts[1]));
                    } else {
                        Platform.runLater(() -> NetworkClient.showErrorMsg("Unknown command", msg, ""));
                    }
               }
           }
           catch (IOException e) {
               e.printStackTrace();
               NetworkClient.showErrorMsg("Connection lost", "", e.getMessage());
           }
        });
